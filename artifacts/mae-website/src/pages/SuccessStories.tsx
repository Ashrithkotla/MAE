        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight max-w-2xl">
              Real people. Real results. Real English.
            </h1>
            <p className="text-lg text-gray-400 max-w-xl leading-relaxed">
              Every student you read about below was once exactly where you are — hesitant, unsure, and held back by English. They took one step. You can too.
            </p>
          </motion.div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {outcomes.map((o, i) => (
              <div key={i} className="text-center" data-testid={`outcome-stat-${i + 1}`}>
                <div className="text-3xl lg:text-4xl font-bold text-gray-900 mb-1">{o.num}</div>
      ...
      <Section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="mb-16">
            ...
            <h2 className="text-4xl font-bold text-gray-900 max-w-xl leading-tight">
              Words from those who transformed.
            </h2>
          </motion.div>
          ...
          <motion.div variants={stagger} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <motion.div
                key={t.id}
                variants={fadeUp}
                className={`rounded-2xl p-7 ${i === 0 ? "bg-primary text-white md:col-span-2 lg:col-span-1" : "bg-gray-50 border border-gray-100"}`}
                data-testid={`testimonial-card-${t.id}`}
              >
                <div className="flex gap-0.5 mb-4">
                  {[1,2,3,4,5].map((s) => (
                    <span key={s} className={`text-sm ${i === 0 ? "text-amber-300" : "text-amber-400"}`}>★</span>
                  ))}
                </div>
                <blockquote className={`text-sm leading-relaxed mb-6 ${i === 0 ? "text-white/90" : "text-gray-700"}`}>
                  "{t.text}"
                </blockquote>
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold ${i === 0 ? "bg-white/20 text-white" : "bg-primary/10 text-primary"}`}>
                    {t.initial}
                  </div>
                  <div>
                    <div className={`text-sm font-semibold ${i === 0 ? "text-white" : "text-gray-900"}`}>{t.name}</div>
                    <div className={`text-xs ${i === 0 ? "text-white/60" : "text-gray-500"}`}>{t.role}</div>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <Section className="py-24 bg-gray-50/50">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="mb-16 text-center">
            ...
            <h2 className="text-4xl font-bold text-gray-900 mb-4">Before and after Maharshi.</h2>
            <p className="text-gray-500 max-w-lg mx-auto">The difference our students experience is not subtle — it is complete.</p>
          </motion.div>

          <motion.div variants={stagger} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {transformations.map((t, i) => (
              <motion.div key={i} variants={fadeUp} className="bg-white rounded-xl border border-gray-100 overflow-hidden" data-testid={`transformation-${i + 1}`}>
                <div className="grid grid-cols-2">
                  ...
                  <div className="p-5 bg-primary/2">
                    <div className="text-xs font-bold text-primary uppercase tracking-wide mb-3">After</div>
                    <p className="text-sm text-gray-800 leading-relaxed font-medium">{t.after}</p>
                  </div>
                </div>
              </motion.div>
            ))}
