...
import { ArrowRight, ChevronDown, Play, Check, MessageCircle, Phone } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main data-testid="home-page">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8 pt-28 pb-20">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <h1 className="text-5xl lg:text-6xl xl:text-7xl font-bold text-gray-900 leading-[1.05] mb-6 tracking-tight">
                English fluency<br />
              ...
              <div className="flex flex-wrap gap-4 mb-14">
                <Link
                  href="/contact"
                  className="inline-flex items-center gap-2 px-6 py-3.5 bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 transition-all hover:shadow-lg hover:shadow-primary/20"
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3.5 border border-gray-200 text-gray-700 font-medium rounded-xl hover:border-gray-300 hover:bg-gray-50 transition-all"
              ...
              <div className="relative">
                ...
                <div className="absolute -bottom-6 -left-6 bg-white rounded-2xl shadow-xl p-4 border border-gray-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-gray-900">Ramesh Maharshi</div>
                      <div className="text-xs text-gray-500">Lead Trainer, 25+ Years</div>
                    </div>
                  </div>
                </div>
                {/* Stars badge */}
                <div className="absolute -top-4 -right-4 bg-white rounded-2xl shadow-xl p-3 border border-gray-100">
                  ...
                  <div className="text-xs font-semibold text-gray-700">98% Satisfaction</div>
                </div>
              </div>
            </motion.div>
          </div>
      ...
      {/* ─── SOCIAL PROOF STRIP ─── */}
      <section className="border-y border-gray-100 bg-gray-50/50 py-8 overflow-hidden" data-testid="social-proof-strip">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-x-10 gap-y-3">
            {["IT Companies", "Banks", "Government Offices", "Schools & Colleges", "BPOs", "MNCs", "Hospitals", "Private Firms"].map((org) => (
              <span key={org} className="text-sm font-medium text-gray-400" data-testid={`social-proof-${org.toLowerCase().replace(/\s+/g, "-")}`}>{org}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <Section className="py-24 bg-white" data-testid="why-choose-section">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="mb-16">
            <div className="text-xs font-semibold text-primary uppercase tracking-widest mb-3">Why Maharshi</div>
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4 max-w-xl leading-tight">
              The institute that delivers results.
            </h2>
            <p className="text-gray-500 max-w-lg">
              Not another grammar class. A transformation program backed by 22 years of proven results.
            </p>
          </motion.div>

          <motion.div variants={stagger} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-gray-100 rounded-2xl overflow-hidden border border-gray-100">
            {features.map((f, i) => (
              <motion.div
                key={f.num}
                variants={fadeUp}
                className={`bg-white p-8 hover:bg-gray-50 transition-colors group ${i === 0 ? "lg:col-span-1 lg:row-span-1" : ""}`}
                data-testid={`feature-card-${i + 1}`}
              >
                <div className="text-xs font-mono text-primary/40 mb-6">{f.num}</div>
                <h3 className="text-lg font-semibold text-gray-900 mb-3 group-hover:text-primary transition-colors">{f.title}</h3>
      ...
      {/* ─── COURSES — EDITORIAL LAYOUT ─── */}
      <Section className="py-24 bg-gray-50/50" data-testid="courses-preview-section">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-16">
            <div>
              ...
              <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 leading-tight">
                Four pathways.<br />One destination.
              </h2>
            </div>
            <Link href="/courses" className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-primary transition-colors whitespace-nowrap" data-testid="courses-view-all">
            ...
            {courses.map((course, i) => (
              <motion.div key={course.id} variants={fadeUp}>
                <Link
                  ...
                  className={`block p-8 rounded-2xl border border-gray-200 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all group ${i === 0 ? "bg-primary text-white border-primary" : "bg-white"}`}
                  data-testid={`course-card-${course.slug}`}
                >
                  <div className={`text-xs font-semibold uppercase tracking-widest mb-4 ${i === 0 ? "text-white/60" : "text-primary"}`}>
                    {course.duration}
                  </div>
                  <h3 className={`text-xl font-bold mb-2 ${i === 0 ? "text-white" : "text-gray-900"}`}>{course.title}</h3>
                  <p className={`text-sm italic mb-6 ${i === 0 ? "text-white/70" : "text-gray-500"}`}>{course.tagline}</p>
                  <p className={`text-sm leading-relaxed mb-8 ${i === 0 ? "text-white/80" : "text-gray-500"}`}>{course.description}</p>
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-medium group-hover:gap-3 transition-all ${i === 0 ? "text-white" : "text-primary"}`}>
                      Learn more
                    </span>
      ...
      {/* ─── JOURNEY TIMELINE ─── */}
      <Section className="py-24 bg-white" data-testid="journey-section">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="text-center mb-16">
            <div className="text-xs font-semibold text-primary uppercase tracking-widest mb-3">Your Journey</div>
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900">How transformation happens</h2>
          </motion.div>
          ...
          <motion.div variants={stagger} className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
            ...
            {journeySteps.map((step) => (
              <motion.div key={step.num} variants={fadeUp} className="text-center relative" data-testid={`journey-step-${step.num}`}>
                <div className="w-20 h-20 rounded-full bg-primary/5 border-2 border-primary/20 flex items-center justify-center mx-auto mb-6 relative z-10 bg-white">
      ...
      {/* ─── GALLERY — MOSAIC ─── */}
      <Section className="py-24 bg-gray-50/50 overflow-hidden" data-testid="gallery-section">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="mb-12">
            <div className="text-xs font-semibold text-primary uppercase tracking-widest mb-3">Inside Maharshi</div>
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 max-w-md">Real classrooms. Real results.</h2>
          </motion.div>

          <motion.div variants={stagger} className="grid grid-cols-12 grid-rows-2 gap-3 h-[480px]">
            <motion.div variants={fadeUp} className="col-span-5 row-span-2 rounded-2xl overflow-hidden" data-testid="gallery-img-1">
              <img src={img2} alt="Classroom session" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
            </motion.div>
            <motion.div variants={fadeUp} className="col-span-4 row-span-1 rounded-2xl overflow-hidden" data-testid="gallery-img-2">
              <img src={img3} alt="Students practicing" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
            </motion.div>
            <motion.div variants={fadeUp} className="col-span-3 row-span-1 rounded-2xl overflow-hidden" data-testid="gallery-img-3">
              <img src={img4} alt="Speaking session" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
            </motion.div>
            <motion.div variants={fadeUp} className="col-span-4 row-span-1 rounded-2xl overflow-hidden" data-testid="gallery-img-4">
              <img src={img5} alt="Group discussion" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
            </motion.div>
            <motion.div variants={fadeUp} className="col-span-3 row-span-1 rounded-2xl overflow-hidden bg-primary flex items-center justify-center p-6" data-testid="gallery-stat">
              <div className="text-center text-white">
                <div className="text-4xl font-bold mb-1">10K+</div>
                <div className="text-sm text-white/70">Lives Changed</div>
              </div>
            </motion.div>
          </motion.div>
        </div>
      ...
      {/* ─── TESTIMONIALS — PULL QUOTE COMPOSITION ─── */}
      <Section className="py-24 bg-white" data-testid="testimonials-section">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="mb-16">
            ...
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 max-w-xl leading-tight">
              Stories that speak louder than promises.
            </h2>
          </motion.div>
          ...
          <motion.div variants={stagger} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <motion.div
              variants={fadeUp}
              className="lg:col-span-2 bg-gray-950 text-white rounded-2xl p-10 relative overflow-hidden"
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <blockquote className="text-xl lg:text-2xl leading-relaxed text-white/90 mb-8 font-light">
                  {testimonials[0].text}
                </blockquote>
            ...
            <motion.div variants={stagger} className="flex flex-col gap-6">
              {testimonials.slice(1, 3).map((t) => (
                <motion.div
                  key={t.id}
                  variants={fadeUp}
                  className="flex-1 bg-gray-50 rounded-2xl p-7 border border-gray-100"
          ...
          <motion.div variants={fadeUp} className="mt-8 text-center">
            <Link href="/success-stories" className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-primary transition-colors" data-testid="testimonials-see-more">
      ...
      <Section className="py-24 bg-gray-50/50" data-testid="blog-preview-section">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-16">
            <div>
              <div className="text-xs font-semibold text-primary uppercase tracking-widest mb-3">From the Blog</div>
              <h2 className="text-4xl lg:text-5xl font-bold text-gray-900">English made simple.</h2>
            </div>
            <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-medium text-gray-600 hover:text-primary transition-colors whitespace-nowrap" data-testid="blog-view-all">
          ...
          <motion.div variants={stagger} className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <motion.div variants={fadeUp} className="lg:col-span-3" data-testid="blog-post-featured">
              <Link href={`/blog/${blogPosts[0].slug}`} className="group block">
                ...
                <h3 className="text-xl lg:text-2xl font-bold text-gray-900 mb-3 group-hover:text-primary transition-colors leading-snug">
                  {blogPosts[0].title}
                </h3>
                <p className="text-sm text-gray-500 leading-relaxed">{blogPosts[0].excerpt}</p>
              </Link>
            </motion.div>
                ...
                <motion.div key={post.id} variants={fadeUp} data-testid={`blog-post-${post.id}`}>
                  <Link href={`/blog/${post.slug}`} className="group flex gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <h3 className="text-sm font-bold text-gray-900 group-hover:text-primary transition-colors leading-snug mb-1 line-clamp-3">
                        {post.title}
                      </h3>
      ...
      <Section className="py-24" data-testid="cta-section">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div
            variants={fadeUp}
            className="relative rounded-3xl bg-[#1a1a2e] overflow-hidden p-12 lg:p-20 text-center"
          >
            ...
            <div className="relative z-10">
              ...
              <p className="text-lg text-white/60 mb-10 max-w-xl mx-auto">
                New batches forming soon. Limited seats per batch. Book your free demo session today.
              </p>
              <div className="flex flex-wrap gap-4 justify-center">
                <Link
                  href="/contact"
                  className="inline-flex items-center gap-2 px-8 py-4 bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 transition-all hover:shadow-lg hover:shadow-primary/30"
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 px-8 py-4 border border-white/20 text-white font-medium rounded-xl hover:border-white/40 hover:bg-white/5 transition-all"
...
function FAQItem({ faq }: { faq: { id: number; question: string; answer: string } }) {
  ...
  return (
    <motion.div variants={fadeUp} className="py-5" data-testid={`faq-item-${faq.id}`}>
      ...
      {open && (
        <p className="mt-4 text-sm text-gray-500 leading-relaxed pr-8">{faq.answer}</p>
      )}
    </motion.div>
  );
}
