...
export default function About() {
  return (
    <main data-testid="about-page" className="min-h-screen">
      <section className="pt-28 pb-20 bg-white dark:bg-gray-950 border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-6xl font-bold text-gray-900 dark:text-white mb-6 leading-tight">22 years of<br />transforming lives.</h1>
              <p className="text-lg text-gray-500 dark:text-gray-400 leading-relaxed mb-8">Maharshi Advanced English is not a classroom. It is a launchpad. Since 2004, our founder Ramesh Maharshi has personally guided over 10,000 students from hesitation to fluency.</p>
            ...
            <div className="relative">
              ...
              <div className="absolute bottom-6 left-6 right-6 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm rounded-xl p-4 border border-gray-100 dark:border-gray-700 shadow-lg">
                <div className="font-bold text-gray-900 dark:text-white text-lg">Ramesh Maharshi</div>
          ...
          <motion.div variants={stagger} className="grid grid-cols-2 lg:grid-cols-4 gap-8">
            {stats.map(s => (
              <motion.div key={s.label} variants={fadeUp} className="text-center" data-testid={`about-stat-${s.label.toLowerCase().replace(/\s+/g, "-")}`}>
                <div className="text-4xl lg:text-5xl font-bold text-white mb-2 tabular-nums"><AnimatedCounter target={s.target} suffix={s.suffix} /></div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div variants={fadeUp}>
              ...
              <h2 className="text-4xl font-bold text-gray-900 dark:text-white mb-6 leading-tight">A teacher who believed English could change destinies.</h2>
              <div className="space-y-4 text-gray-600 dark:text-gray-300 leading-relaxed">
                ...
                <p>His students were engineers who froze in meetings, students who failed interviews despite knowing the answers, homemakers who couldn't communicate with their children's schools, and professionals passed over for promotions because of a language barrier.</p>
                ...
                <p>Over 22 years and 10,000+ students later, that conviction hasn't changed. Ramesh Maharshi still teaches personally. Still knows every student's name. Still cares about outcomes, not enrollment numbers.</p>
              </div>
            </motion.div>
            <motion.div variants={fadeUp} className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-primary rounded-2xl flex items-center justify-center p-6 text-center">
                <div><div className="text-3xl font-bold text-white mb-1">22+</div><div className="text-xs text-white/70">Years of Teaching</div></div>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <Section className="py-24 bg-gray-50/50 dark:bg-gray-900/50">
        <div className="max-w-4xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="text-center mb-16">
            ...
            <h2 className="text-4xl font-bold text-gray-900 dark:text-white">Two decades of milestones.</h2>
          </motion.div>
          <motion.div variants={stagger} className="relative">
            <div className="absolute left-[92px] top-0 bottom-0 w-px bg-gray-200 dark:bg-gray-700 hidden sm:block" />
            {timeline.map((item, i) => (
              <motion.div key={i} variants={fadeUp} className="flex gap-8 mb-10 last:mb-0" data-testid={`timeline-item-${i + 1}`}>
                <div className="flex-shrink-0 w-20 text-right">
                  <span className="text-sm font-bold text-primary bg-primary/5 dark:bg-primary/10 px-2 py-1 rounded-lg">{item.year}</span>
                </div>
      ...
      <Section className="py-24 bg-white dark:bg-gray-950">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="mb-12">
            ...
            <h2 className="text-4xl font-bold text-gray-900 dark:text-white max-w-xl leading-tight">What we stand for.</h2>
          </motion.div>
          <motion.div variants={stagger} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {missions.map((m, i) => (
              <motion.div key={i} variants={fadeUp} className="flex items-start gap-4 p-5 rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-800" data-testid={`mission-item-${i + 1}`}>
                <div className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                  <div className="w-2 h-2 rounded-full bg-primary" />
                </div>
                <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">{m}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
