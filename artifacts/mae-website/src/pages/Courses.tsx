      ...
      <section className="pt-28 pb-16 bg-white dark:bg-gray-950 border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-5xl lg:text-6xl font-bold text-gray-900 dark:text-white mb-6 max-w-2xl leading-tight">Find the program that fits your goal.</h1>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={stagger} className="space-y-8">
            {courses.map((course, i) => {
              ...
              return (
                <motion.div key={course.id} variants={fadeUp} className={`rounded-2xl border overflow-hidden ${i === 0 ? "border-primary/20 dark:border-primary/30 bg-primary/2 dark:bg-primary/5" : "border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900"}`} data-testid={`course-listing-${course.slug}`}>
                  <div className="p-8 lg:p-10">
                    <div className="grid lg:grid-cols-3 gap-8 items-start">
                      <div className="lg:col-span-2">
                        <div className="flex items-center gap-3 mb-4">
                          <span className={`text-xs font-bold uppercase tracking-widest px-2.5 py-1 rounded-md ${i === 0 ? "bg-primary/10 text-primary" : "bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400"}`}>{course.duration}</span>
                        </div>
                        <h2 className="text-2xl lg:text-3xl font-bold text-gray-900 dark:text-white mb-2">{course.title}</h2>
                        <p className="text-base italic text-gray-500 dark:text-gray-400 mb-5">{course.tagline}</p>
                        <p className="text-gray-600 dark:text-gray-300 leading-relaxed mb-6">{course.description}</p>
                        <div className="flex flex-wrap gap-2">
                          {course.features.map(f => (
                            <span key={f} className="text-xs px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 font-medium">{f}</span>
                          ))}
                        </div>
                      </div>
                      <div className="space-y-4">
                        <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-5 space-y-3">
                          <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-300">
                            <Clock size={14} className="text-gray-400" /><span>{course.duration}</span>
                          </div>
                          <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-300">
                            <Users size={14} className="text-gray-400" /><span>{meta.batchSize}</span>
                          </div>
                          <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-300">
                            <Wifi size={14} className="text-gray-400" /><span>{meta.mode}</span>
                          </div>
                        </div>
                        <Link href={`/courses/${course.slug}`} className="flex items-center justify-center gap-2 w-full py-3.5 bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 transition-colors" data-testid={`course-learn-more-${course.slug}`}>
                          Learn More <ArrowRight size={15} />
                        </Link>
                        <a href="https://wa.me/917286066661?text=Hi! I want to enroll in the course." target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-3.5 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-medium rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors text-sm" data-testid={`course-whatsapp-${course.slug}`}>
      ...
      <section className="py-20 bg-gray-950 text-white">
        <div className="max-w-3xl mx-auto px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            <a href="tel:+917286066661" className="inline-flex items-center gap-2 px-6 py-3.5 bg-primary text-white font-semibold rounded-xl hover:bg-primary/90 transition-colors" data-testid="courses-cta-call">Call +91 7286 066 661</a>
