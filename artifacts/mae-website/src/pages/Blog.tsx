    ...
    <main data-testid="blog-page" className="min-h-screen">
      <section className="pt-28 pb-16 bg-white dark:bg-gray-950 border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <h1 className="text-5xl lg:text-6xl font-bold text-gray-900 dark:text-white mb-6 leading-tight max-w-2xl">English insights. Practical tips. Real guidance.</h1>
      ...
      <Section className="py-16 bg-white dark:bg-gray-950">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <motion.div variants={fadeUp} className="mb-16" data-testid="blog-featured">
            <Link href={`/blog/${blogPosts[0].slug}`} className="group grid lg:grid-cols-2 gap-8 items-center">
              ...
              <div>
                ...
                <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 dark:text-white mb-4 leading-tight group-hover:text-primary transition-colors">{blogPosts[0].title}</h2>
                <p className="text-gray-500 dark:text-gray-400 leading-relaxed mb-6">{blogPosts[0].excerpt}</p>
          ...
          <div className="h-px bg-gray-100 dark:bg-gray-800 mb-16" />
            ...
            {blogPosts.map(post => (
              <motion.div key={post.id} variants={fadeUp} data-testid={`blog-card-${post.id}`}>
                <Link href={`/blog/${post.slug}`} className="group block">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2 leading-snug group-hover:text-primary transition-colors">{post.title}</h3>
                  <p className="text-sm text-gray-500 dark:text-gray-400 leading-relaxed line-clamp-3">{post.excerpt}</p>
                  <div className="mt-4 flex items-center gap-2 text-xs font-medium text-gray-400 dark:text-gray-500">
                    <span>{post.author}</span><span>·</span><span>{post.publishDate}</span>
                  </div>
                </Link>
              </motion.div>
            ))}
