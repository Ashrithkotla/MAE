  ...
  return (
    <main data-testid="blog-post-page" className="min-h-screen">
      ...
      <section className="pt-24 pb-12 bg-white border-b border-gray-100">
        <div className="max-w-3xl mx-auto px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <Link href="/blog" className="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-900 mb-8 transition-colors" data-testid="post-back">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-gray-500">Founder, Maharshi Advanced English</div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section className="py-12 bg-white">
        ...
        >
          <div
            className="prose prose-lg prose-gray max-w-none prose-headings:font-bold prose-headings:text-gray-900 prose-p:text-gray-600 prose-p:leading-relaxed prose-strong:text-gray-900"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {others.map((p) => (
                <Link key={p.id} href={`/blog/${p.slug}`} className="group block" data-testid={`related-post-${p.id}`}>
                  ...
                  <h3 className="text-sm font-bold text-gray-900 group-hover:text-primary transition-colors leading-snug">{p.title}</h3>
                </Link>
              ))}
            </div>
          </div>
